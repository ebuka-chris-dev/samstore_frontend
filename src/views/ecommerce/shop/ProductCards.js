  ...
  const renderProducts = () => {
    if (products.length) {
      return products?.map(item => {
        return (
          <Link to={`/ecommerce/product-detail/${item._id}`} key={item._id} style={{textDecoration: 'none'}}>
          <Card className='ecommerce-card' key={item._id}>
            <div className='item-img text-center mx-auto'>
              
                <img className='img-fluid card-img-top' src={item.featured_image} alt={item.title} />
            ...
            <CardBody>
              ...
              <CardText className='item-description' style={{color: 'gray'}} dangerouslySetInnerHTML={{ __html: item.description }} />
            </CardBody>
