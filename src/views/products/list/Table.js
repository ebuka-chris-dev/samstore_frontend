...
// ** Store & Actions 
import { getProducts, deleteProduct } from '../../ecommerce/store'
import { useDispatch, useSelector } from 'react-redux'

// ** Third Party Components
import ReactPaginate from 'react-paginate'
import DataTable from 'react-data-table-component'
import { ChevronDown,  Edit2, Trash2, Plus } from 'react-feather'
          ...
          <div className='d-flex align-items-center table-header-actions'>
            {/* <UncontrolledDropdown className='me-1'>
              ...
              <DropdownMenu>
                ...
                <DropdownItem className='w-100' onClick={() => downloadCSV(store.products.data)}>
...
const renderClient = row => {
  return (
    <>
      {/* Featured Image */}
      <Link to={`/ecommerce/product-detail/${row._id}`}>
        <Avatar
          className='me-1'
      ...
      {row.images?.map((e, i) => (
        <Link
          key={i}
          to={`/ecommerce/product-detail/${row._id}`}
        >
          <Avatar
            className='me-1'
            img={e}
...
const UsersList = () => {
  ...
  const CustomPagination = () => {
    const count = Number(Math.ceil(store.pagination.total / rowsPerPage))
  ...
  const handleEditProduct = e => {
    setTitle(e.title)
    setFeaturedImage(e.featured_image)
    setOtherImages(e.images)
          ...
          <DataTable
            ...
            progressComponent={
              <div className='py-3 text-center'>
                <Spinner />
                <div className='mt-1'>Loading products...</div>
              </div>
            }
            subHeaderComponent={
              <CustomHeader
